...
/*   Updated: 2023/01/06 17:32:37 by minjinki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_queue	*init_queue(t_queue *q, int h, int w)
{
	...
	if (!q)
		print_error("Fail to allocate memory: fail to init queue\n");
...
{
	if (!(q->data[q->rear]))
		q->data[q->rear] = ft_calloc(2, sizeof(int));
	if (!(q->data[q->rear]))
		print_error("Fail to allocate memory: fail to enqueue\n");
	q->data[q->rear][0] = i;
	q->data[q->rear][1] = j;
