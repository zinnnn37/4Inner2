...
/*   Updated: 2023/02/09 11:33:50 by minjinki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	a_to_b2(t_info *info, int min, int mid, int max)
{
	int	top;
	int	bottom;
	...
	top = range_from_top(info->a, min, max);
	bottom = range_from_bottom(info->a, min, max) + 1;
	if (top <= bottom)
	{
		while (top-- > 0)
			rn(info->a, "ra");
